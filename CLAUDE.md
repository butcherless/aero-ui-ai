...
- **Pagination**: every `*Api.list()` fetches `pageSize=20` (`Http.defaultPageSize`) via `page`/`pageSize` query params — the backend caps `pageSize` at 100 and defaults to 20 itself. List responses carry no total count, so `EntityCrudPage`/`FlightInstancesPage`/`RoutesPage` treat "Next" as enabled whenever the last fetch returned a full page (`items.size >= pageSize`); there's no way to know the real last page ahead of time.
...
**Why Routes and Flight Instances are special-cased**: the backend has a real list-all/search-by-origin/destination/both endpoint for routes (`GET /api/v1/routes`), but still no get-by-key/update/delete for the route entity itself (only create, that list/search endpoint, and the airline-association sub-resource endpoints), and Flight Instances is read-only (no create/update/delete at all). Don't assume a uniform pattern across all 7 pages — check which shape a given entity's endpoints actually support before copying a pattern from another page. `FlightInstancesPage` and `RoutesPage` both duplicate `EntityCrudPage`'s pagination state (`page`/`hasNext` Vars, the `Pagination` component) by hand rather than sharing it, since neither is built on `EntityCrudPage`; `RoutesPage` additionally supports two independent, simultaneously-usable origin/destination filters, since — unlike Airports' name/country filters — the routes endpoint supports combining both at once.

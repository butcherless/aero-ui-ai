...
/** Covers `EntityCrudPage.readOnly`, the counterpart to `EntityCrudPageSpec`/`EntityCrudPageLoadSpec` (which only
  ...
  * removes the detail panel again. Async (like `EntityCrudPageLoadSpec`) since the initial list only appears once
...
class EntityCrudPageReadOnlySpec extends LaminarAsyncMountSpec {
  ...
  it("renders no detail panel and no \"+ Add\" button before anything is picked") {
  ...
  it("selecting a row shows its fields as static text, and Close removes the detail panel again") {
    val promise = Promise[List[Item]]()
    val root = buildPage(promise)
    promise.success(List(Item("1", "Alpha")))
